...
import { ReactRefreshScript } from "./dev/react-refresh-script";
	...
	return (
		<>
			{import.meta.env.ROUTER_MODE === "server" ? (
				<ServerAssets />
			) : import.meta.env.SSR ? // <ClientAssets />
			null : null}
			{import.meta.env.DEV ? (
				<>
					<ReactRefreshScript />
...
export async function ServerAssets() {
	const allAssets = [
		...new Set([
			...(await getApp().webServer.context.findAssets(request())),
		]).values(),
	];
	return (
		<>
...
// 	let modules = await import.meta.app.getDependenciesForURL(request());
// 	return [
// 		...new Set([...(await import.meta.app.findAssets(request()))]).values(),
// 	];
// };
